...
function generatePaper(subject, selectedChapters, paperType) {
    ...
    if (paperType === 'subjective' || paperType === 'both') {
        ...
        // Short Questions
        let shortQs = [];
        selectedChapters.forEach(ch => shortQs = shortQs.concat(QUESTIONS[subject][ch].shortQuestions));
        ...
        y += 8;
        shortQs.forEach((q,i) => { if(y>270){doc.addPage(); y=20;} doc.text(`${i+1}. ${q}`, margin, y); y+=10; });

        y += 5;

        // Long Questions
        let longQs = [];
        selectedChapters.forEach(ch => longQs = longQs.concat(QUESTIONS[subject][ch].longQuestions));
        longQs = shuffleArray(longQs).slice(0,5);

        doc.text('Long Questions (Attempt any 3, 5 marks each)', margin, y);
        y += 8;
        longQs.forEach((q,i) => { if(y>270){doc.addPage(); y=20;} doc.text(`${i+1}. ${q}`, margin, y); y+=15; });
    }
