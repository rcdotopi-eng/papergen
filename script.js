...
function generatePaper(subject, selectedChapters, paperType) {
    ...
    if (paperType === 'objective' || paperType === 'both') {
        ...
        // Combine MCQs from selected chapters
        let mcqs = [];
        ...
        mcqs = shuffleArray(mcqs).slice(0, 10); // 10 MCQs

        doc.setFont('times', 'normal');
        mcqs.forEach((q, i) => {
            if (y > 270) { doc.addPage(); y = 20; }
            // Split long questions into multiple lines if needed
            let lines = doc.splitTextToSize(`${i + 1}. ${q}`, pageWidth - 2 * margin);
            doc.text(lines, margin, y);
            y += lines.length * 6 + 4; // spacing between questions
        });
    ...
    if (paperType === 'subjective' || paperType === 'both') {
        ...
        shortQs = shuffleArray(shortQs).slice(0, 10); // 10 short questions
        ...
        longQs = shuffleArray(longQs).slice(0, 5); // pick 5, students attempt 3
